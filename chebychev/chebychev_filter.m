function H = chebychev_filter(omega0, N, epsilon)
...
for k = 0:(N-1)
    ...
    H_expr = H_expr * (v_k*(s - u_k))/(u_k*(s - v_k));
end

% Simplify the expression before returning
H = simplify(H_expr);

% Define a symbolic variable omega for frequency
syms omega real

% Substitute s = j*omega into H(s)
H = subs(H, s, 1j*omega);
end
