function [SO_signal, Cos_signal,angular_freqs_SO, angular_freqs_COS] = generate_signals(freq_scaling,amp_scaling)
...
angular_freqs_COS = 0.9; %angular_freqs_SO(end);

t_0 = 250;
T = 100;

Cos_normalization = 25/7;
SO_normalizaion = 1;
...
SO_signal = @(t)  conj(SO_normalizaion*sum( amps_SO .* exp(1i * angular_freqs_SO .* t),2).*exp(-(t-t_0).^2./T^2));
end
