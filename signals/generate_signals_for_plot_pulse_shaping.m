function [SO_signal, Cos_signal,angular_freqs_SO, angular_freqs_COS] = generate_signals_for_plot_pulse_shaping(freq_scaling,amp_scaling)
...
Cos_signal = @(t)   -real(Cos_normalization .* cos(angular_freqs_COS .* t).*2.*(t-t_0)./T^2.*exp(-(t-t_0).^2./T^2));

% Function handle for superoscillatory signal
SO_signal = @(t)  real(sum( amps_SO .* exp(1i * angular_freqs_SO .* t),2).*2.*(t-t_0)./T^2.*exp(-(t-t_0).^2./T^2));
end
